...
- [docs/features/invite-code-api-design.md](file://docs/features/invite-code-api-design.md)
...
- [ui/src/ui/invite-code-client.ts](file://ui/src/ui/invite-code-client.ts)
- [ui/src/ui/app-invite-code.ts](file://ui/src/ui/app-invite-code.ts)
...
UI["Web控制面<br/>ui/src/ui/app.ts"]
InviteCodeClient["邀请码客户端<br/>ui/src/ui/invite-code-client.ts"]
ProfileView["Profile视图<br/>ui/src/ui/views/profile.ts"]
end
Entry --> Boot
Boot --> Auth
Boot --> Call
...
- [ui/src/ui/app.ts:431-437](file://ui/src/ui/app.ts#L431-L437)
- [ui/src/ui/invite-code-client.ts:125-230](file://ui/src/ui/invite-code-client.ts#L125-L230)
- [ui/src/ui/views/profile.ts:858-919](file://ui/src/ui/views/profile.ts#L858-L919)
...
- **新增**：邀请码系统（Invite Code System）
  - 职责：提供API密钥验证与配置分发，支持邀请码格式验证、HMAC-SHA256签名认证、防重放攻击
  - 关键点：前端UI集成、API客户端实现、安全签名算法、错误处理与状态管理
...
- [docs/features/invite-code-api-design.md:1-140](file://docs/features/invite-code-api-design.md#L1-L140)
...
WS --> InviteCode["邀请码系统<br/>API密钥验证"]
subgraph "安全与运行时"
Auth["认证/鉴权/配对"]
Sandbox["沙箱执行/工具策略"]
Env["环境变量/配置热重载"]
InviteAuth["HMAC-SHA256签名认证"]
InviteValidation["邀请码格式验证"]
end
WS --> Auth
Agents --> Sandbox
WS --> Env
InviteCode --> InviteAuth
InviteCode --> InviteValidation
```
...
- [docs/features/invite-code-api-design.md:62-140](file://docs/features/invite-code-api-design.md#L62-L140)
...
### 前端UI层集成
邀请码系统在Web控制面的Profile页面中提供完整的用户交互体验：

- **UI状态管理**：在OpenClawApp类中维护邀请码验证状态（inviteCode、inviteCodeVerifying、inviteCodeVerified、inviteCodeError、llmApiKey、llmModel）
- **表单组件**：提供邀请码输入框和验证按钮，支持实时格式验证和状态反馈
- **错误处理**：集成用户友好的错误消息显示，支持多种业务错误场景
- **成功反馈**：验证成功后显示API Key和模型信息摘要

### API客户端层实现
邀请码客户端提供完整的API调用能力：

- **HMAC-SHA256签名**：实现标准的HMAC-SHA256签名算法，支持浏览器和Node.js环境
- **防重放保护**：集成时间戳（5分钟有效期）和随机nonce机制
- **请求头管理**：自动处理X-App-Id、X-Timestamp、X-Nonce、X-Signature等必需请求头
- **响应处理**：解析业务响应码，提取API Key配置信息

### 后端网关层集成
邀请码系统通过HTTP API与网关集成：

- **认证机制**：基于App ID和App Secret的HMAC-SHA256签名认证
- **安全策略**：时间戳验证、签名验证、防重放攻击
- **配置分发**：根据邀请码返回相应的API Key配置
- **错误码管理**：标准化的错误码返回机制

```mermaid
sequenceDiagram
participant User as "用户界面"
participant UI as "Web控制面"
participant Client as "邀请码客户端"
participant API as "邀请码API"
participant Gateway as "网关控制平面"
User->>UI : "输入邀请码"
UI->>Client : "验证邀请码格式"
Client->>Client : "生成时间戳和nonce"
Client->>Client : "计算HMAC-SHA256签名"
Client->>API : "POST /api/v1/app/member/invite-code/redeem"
API->>API : "验证App ID/时间戳/签名"
API->>API : "查询邀请码状态"
API-->>Client : "返回API Key配置"
Client-->>UI : "返回验证结果"
UI-->>User : "显示成功/错误信息"
```

**图示来源**
- [ui/src/ui/app-invite-code.ts:31-106](file://ui/src/ui/app-invite-code.ts#L31-L106)
- [ui/src/ui/invite-code-client.ts:137-201](file://ui/src/ui/invite-code-client.ts#L137-L201)
- [docs/features/invite-code-api-design.md:390-421](file://docs/features/invite-code-api-design.md#L390-L421)

**章节来源**
- [ui/src/ui/app.ts:431-437](file://ui/src/ui/app.ts#L431-L437)
- [ui/src/ui/app-invite-code.ts:1-186](file://ui/src/ui/app-invite-code.ts#L1-L186)
- [ui/src/ui/invite-code-client.ts:1-230](file://ui/src/ui/invite-code-client.ts#L1-L230)
- [ui/src/ui/views/profile.ts:858-919](file://ui/src/ui/views/profile.ts#L858-L919)
- [docs/features/invite-code-api-design.md:1-448](file://docs/features/invite-code-api-design.md#L1-L448)
...
UI["Web控制面"] --> InviteCodeClient["邀请码客户端"]
InviteCodeClient --> API["邀请码API"]
API --> Gateway["网关控制平面"]
```
...
- **新增**：邀请码验证缓存：建议在客户端实现轻量级缓存，避免频繁重复验证
...
- **新增**：邀请码系统故障排除
  - 邀请码格式验证失败：检查是否符合BOSS-XXXX-XXXX格式
  - 签名认证失败：确认App ID/Secret配置正确，时间戳在有效期内
  - API Key配置缺失：检查邀请码状态和绑定的API Key配置
...
- [docs/concepts/architecture.md:93-109](file://docs/concepts/architecture.md#L93-L109)
- [docs/features/invite-code-api-design.md:128-140](file://docs/features/invite-code-api-design.md#L128-L140)
...
**新增**：邀请码系统的集成进一步增强了系统的安全性和可扩展性，通过标准化的API密钥管理和认证机制，为用户提供了便捷的访问控制解决方案。

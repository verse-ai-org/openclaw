...
## 更新摘要
**所做更改**
- 新增分段控制组件集成章节，介绍新的技能分类界面
- 更新技能分类功能，从简单的过滤器升级为现代化的分段控制
- 增加技能来源过滤器的详细说明
- 更新架构图以反映新的组件结构
...
**更新** 集成了新的分段控制组件，提供更现代化的技能分类界面，支持按技能来源进行智能分类和筛选。
...
UI_Lit[Lit HTML 组件]
UI_React[React 组件]
SegmentedControl[分段控制组件]
SkillCategoryPills[技能分类胶囊]
end
subgraph "控制器层"
Ctrl_Skills[技能控制器]
...
UI_React --> SegmentedControl
SegmentedControl --> SkillCategoryPills
...
- [SkillCategoryPills.tsx:1-17](file://ui-react/src/components/skills/SkillCategoryPills.tsx#L1-L17)
...
4. **分段控制组件** - **新增** 提供现代化的技能分类界面
5. **技能分类胶囊** - **新增** 基于分段控制的技能分类展示
...
**更新** 新增的分段控制组件提供了更直观的技能分类体验，支持按技能来源进行快速筛选。
...
participant SegmentedControl as 分段控制
participant CategoryPills as 分类胶囊
...
UI->>SegmentedControl : 设置活动分类
SegmentedControl->>CategoryPills : 渲染分类选项
CategoryPills->>Controller : 触发分类筛选
Controller->>Gateway : skills.status 请求
Gateway->>Server : 发送筛选请求
Server->>Agent : 构建筛选后的技能状态
Agent-->>Server : 返回筛选结果
Server-->>Gateway : 筛选后的技能数据
Gateway-->>Controller : 响应结果
Controller-->>UI : 更新界面状态
UI-->>User : 显示筛选后的技能列表
User->>UI : 启用/禁用技能
...
- [SkillsPage.tsx:181-201](file://ui-react/src/pages/SkillsPage.tsx#L181-L201)
...
class SkillsSourceFilter {
+all : "all"
+openclawBundled : "openclaw-bundled"
+openclawManaged : "openclaw-managed"
+openclawExtra : "openclaw-extra"
}
...
SkillsSourceFilter --> SkillStatusEntry : filters
```

**图表来源**
- [skills.ts:4-24](file://ui/src/ui/controllers/skills.ts#L4-L24)
- [types.ts:577-612](file://ui/src/ui/types.ts#L577-L612)
- [SkillsPage.tsx:25-33](file://ui-react/src/pages/SkillsPage.tsx#L25-L33)
...
SegmentedControl --> Option : contains
SkillCategoryPills --> Category : renders
SkillCategoryPills --> SegmentedControl : uses
```

**图表来源**
- [SkillCategoryPills.tsx:3-17](file://ui-react/src/components/skills/SkillCategoryPills.tsx#L3-L17)
...
- [SkillCategoryPills.tsx:1](file://ui-react/src/components/skills/SkillCategoryPills.tsx#L1)
...
- **缓存策略**：使用内存缓存减少重复请求
- **并发处理**：支持多个技能状态的并行检查
- ****新增** 分段控制优化**：分段控制组件使用虚拟滚动优化大量分类选项的渲染
...
### **新增** 分段控制性能优化
- **虚拟滚动**：对于大量技能分类选项，使用虚拟滚动技术提升渲染性能
- **防抖处理**：分类切换操作使用防抖机制，避免频繁的 API 调用
- **本地过滤**：支持本地技能状态过滤，减少不必要的网关请求
...
5. ****新增** 现代化界面** - 集成分段控制组件，提供更直观的技能分类体验

**更新** 最新的分段控制组件集成显著提升了技能工具栏的用户体验，通过现代化的界面设计和智能的技能分类功能，用户可以更高效地管理和查找所需的技能。

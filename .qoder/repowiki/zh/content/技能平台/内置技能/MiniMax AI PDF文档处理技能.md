...
- [design.md](file://skills/minimax-pdf/design/design.md)
</cite>

## 更新摘要
**所做更改**
- 更新了文档以反映minimax-pdf技能已被完全移除的事实
- 移除了所有与该技能相关的技术细节和实现说明
- 更新了项目结构和架构概览部分以移除该技能的相关内容
- 删除了所有具体的技术实现细节和代码示例
...
**已更新** 该技能已被完全移除，不再存在于当前代码库中。

MiniMax AI PDF文档处理技能曾经是一个功能强大的PDF生成和处理系统，专注于创建视觉上精美的PDF文档。该技能提供了三种主要工作流程：从零创建新文档（CREATE）、填写现有PDF表单字段（FILL）和重新格式化现有文档（REFORMAT）。

由于该技能已被完全移除，本文档将不再提供任何技术实现细节或使用说明。

## 项目结构

**已更新** 该技能已被完全移除，项目结构中不再包含minimax-pdf目录。

该技能曾经采用模块化架构，包含以下核心组件：
...
**已更新** 该技能已被完全移除，核心组件不再可用。

### 设计令牌系统

设计令牌系统曾经是整个PDF生成管道的核心，负责从文档元数据推导出完整的视觉设计规范：
...
**已更新** 该技能已被完全移除，架构概览不再适用。

MiniMax PDF技能曾经采用分层架构设计，确保每个组件职责明确且可独立扩展：
...
**已更新** 该技能已被完全移除，详细组件分析不再适用。

### 设计令牌生成器（palette.py）

设计令牌生成器曾经是整个系统的创意核心，负责将抽象的文档元数据转换为具体的视觉设计规范：
...
设计令牌生成器曾经支持15种不同的文档类型，每种类型都有独特的色彩方案、字体组合和视觉模式：
...
封面渲染系统曾经采用HTML+CSS+JavaScript的混合架构，确保跨平台一致性和高质量输出：
...
文档重构系统曾经能够将现有文档转换为content.json格式，然后应用设计系统重新生成PDF：
...
**已更新** 该技能已被完全移除，依赖关系分析不再适用。

MiniMax PDF技能的依赖关系曾经呈现清晰的层次结构：
...
**已更新** 该技能已被完全移除，性能考虑不再适用。

MiniMax PDF技能在设计时曾经充分考虑了性能优化：
...
**已更新** 该技能已被完全移除，不再提供任何功能或服务。

MiniMax AI PDF文档处理技能曾经是一个功能完整、设计精良的PDF生成系统。它通过创新的token-based设计管道，实现了从概念到成品的完整自动化流程。

然而，由于该技能已被完全移除，现在无法提供任何使用指导或技术支持。用户需要寻找替代的PDF生成解决方案来满足文档处理需求。

...
- [skills/xiaohongshu/SKILL.md](file://skills/xiaohongshu/SKILL.md)
</cite>
...
## 简介
本文件系统性梳理 OpenClaw 提供的内置技能，覆盖消息平台类（GitHub、Discord、Telegram、Slack、WhatsApp、iMessage）、生产力工具类（Notion、Obsidian、1Password）、媒体处理类（OpenAI 图像生成、Whisper 语音转文字、TTS 语音合成）与系统工具类（tmux、weather、spotify 等），以及新增的旅行规划技能和 Xiaohongshu 集成模块。为使用者提供功能特性、配置参数、使用场景、最佳实践、调用示例、权限要求与故障排除指引。
...
M["skills/travel-planner/SKILL.md"]
N["skills/xiaohongshu/SKILL.md"]
end
subgraph "运行时要求"
BIN["二进制依赖"]
ENV["环境变量"]
...
- [skills/travel-planner/SKILL.md:1-722](file://skills/travel-planner/SKILL.md#L1-L722)
- [skills/xiaohongshu/SKILL.md:1-310](file://skills/xiaohongshu/SKILL.md#L1-L310)
...
- 旅行规划技能（全新JS实现）
  - 旅行规划：提供多日行程规划、路线框定、交通酒店策略、预算打包、行前/在途调整的完整工作流。
  - Xiaohongshu 集成：通过小红书搜索和证据提取，为旅行路线提供高质量的用户生成内容参考。
- 新增技能
  - Xiaohongshu：支持内容检索、发布、互动等完整功能，包括多账号管理和远程 CDP 支持。
...
### 旅行规划技能（全新JavaScript实现）

**更新** 旅行规划技能已从 Python 完全迁移到 JavaScript/Node.js 平台

- 功能特性
  - 多日行程规划、路线框定、交通酒店策略、预算打包、行前/在途调整
  - Xiaohongshu 集成提供高质量用户生成内容参考
  - 国内旅行优先策略，国际旅行基础能力
  - 完整的旅行数据库管理与状态跟踪
- 配置参数
  - 二进制依赖：node（用于脚本执行）
  - 数据库路径：~/.openclaw/agents/travel-planner/
  - 可选技能：flyai、12306、amap-lbs-skill（预订阶段验证）
- 使用场景
  - 3 天及以上行程规划、路线对比、交通酒店组合策略
  - 与真实行程绑定的预算、打包、礼仪、安全、行前清单
  - 行中改线、错过交通后的重排与当天应急建议
- 最佳实践
  - 优先使用 Xiaohongshu 证据进行路线框定
  - 明确的兜底原则：伙伴技能或实时检查失败时仍可给路线框定
  - 严格的顺序执行：证据获取 → 路线框定 → 用户确认 → 预订准备
- 示例路径
  - 偏好采集与行程建档：[skills/travel-planner/SKILL.md:62-98](file://skills/travel-planner/SKILL.md#L62-L98)
  - Xiaohongshu 证据构建：[skills/travel-planner/SKILL.md:100-125](file://skills/travel-planner/SKILL.md#L100-L125)
  - 路线框定与选择：[skills/travel-planner/SKILL.md:127-175](file://skills/travel-planner/SKILL.md#L127-L175)
  - 预订验证与准备：[skills/travel-planner/SKILL.md:187-245](file://skills/travel-planner/SKILL.md#L187-L245)
- 权限要求
  - 旅行数据库文件读写权限
  - 可选：flyai、12306、amap-lbs-skill 等技能的访问权限
- 故障排除
  - Xiaohongshu 不可用：标记 source_reason，进入可解释降级
  - 预订验证失败：明确说明失败原因，仍可给路线框定但价格/余票/时刻标注为未验证
  - 数据库问题：检查 ~/.openclaw/agents/travel-planner/ 目录权限

**全新实现细节**

旅行规划技能已完全从 Python 迁移到 JavaScript/Node.js 平台，采用现代化的 ESM 模块系统和标准化 CLI 接口。主要变化包括：

1. **运行时迁移**：从 Python 运行时迁移到 Node.js 运行时
2. **数据库管理**：使用 JavaScript 版本的数据库管理器（db.mjs），替代原有的 travel_db.py
3. **CLI 接口**：所有脚本现在使用统一的 CLI 参数格式 `--key=value`
4. **模块化设计**：采用 ES6 模块系统，每个功能模块独立导出
5. **类型安全**：使用 JavaScript 的类型检查和更好的错误处理
6. **测试支持**：完整的单元测试覆盖，使用 Vitest 测试框架
7. **文件结构**：采用现代 Node.js 项目结构，包含 package.json 和 ESM 模块
...
**更新** 新增 Xiaohongshu 集成技能，支持内容检索、发布、互动等完整功能

- 功能特性
  - 内容检索：首页推荐、关键词搜索、笔记详情获取
  - 发布功能：图文/视频发布，支持多账号管理
  - 互动操作：评论、点赞、收藏、用户主页信息
  - 测试浏览器：无头/有头模式，登录状态检查
- 配置参数
  - 二进制依赖：python3、Chrome 浏览器
  - 账号管理：多账号配置与切换
  - 远程支持：CDP 远程连接与调试
- 使用场景
  - 内容发现与研究：获取旅行灵感和攻略
...
- 最佳实践
  ...
  - 图文发布时必须有图片，视频发布时必须有视频
  - 默认使用无头模式；未登录时切换有窗口模式登录
  - 支持远程 CDP 连接，便于服务器部署
- 示例路径
  - 测试浏览器流程：[skills/xiaohongshu/SKILL.md:36-113](file://skills/xiaohongshu/SKILL.md#L36-L113)
  - 图文发布流程：[skills/xiaohongshu/SKILL.md:43-55](file://skills/xiaohongshu/SKILL.md#L43-L55)
  - 内容检索与互动：[skills/xiaohongshu/SKILL.md:57-237](file://skills/xiaohongshu/SKILL.md#L57-L237)
  - 多账号发布：[skills/xiaohongshu/SKILL.md:197-204](file://skills/xiaohongshu/SKILL.md#L197-L204)
- 权限要求
  - 小红书账号登录权限
  - Chrome 浏览器访问权限
  - 文件系统读写权限（用于临时文件）
- 故障排除
  - 登录失败：重新扫码登录；使用 get-login-qrcode 获取二维码
  - 图片/视频下载失败：更换 URL 或使用本地文件
  - 页面选择器失效：检查 scripts/cdp_publish.py 中的选择器
  - 远程连接问题：检查 CDP 地址和端口配置
...
## 依赖关系分析
- 运行时依赖
  - 二进制依赖：gh、tmux、curl、whisper、spogo/spy-spotify_player、obsidian-cli、python3、op、node
  - 环境变量：NOTION_API_KEY、OPENAI_API_KEY、SHERPA_ONNX_RUNTIME_DIR、SHERPA_ONNX_MODEL_DIR
  - 操作系统：部分技能限定 macOS/Linux/Windows
- 配置与权限
  - 令牌/密钥：Discord、Slack、Notion、1Password、OpenAI、Spotify、Xiaohongshu
  - 门控：Discord actions.* 默认可能关闭
  - 旅行规划：需要旅行数据库目录 ~/.openclaw/agents/travel-planner/
- 外部服务
  - GitHub API、Discord API、Slack API、Notion API、OpenAI API、Spotify Web API、wttr.in、Xiaohongshu API
...
- 技能特定
  ...
  - 旅行规划：检查旅行数据库目录权限；Xiaohongshu 不可用时标记 source_reason
  - Xiaohongshu：检查登录状态；使用 get-login-qrcode 获取二维码；支持远程 CDP 连接
...
## 结论
OpenClaw 的内置技能覆盖消息平台、生产力工具、媒体处理与系统工具四大类，以及新增的旅行规划和 Xiaohongshu 集成功能。通过 SKILL.md 明确了运行时依赖、配置要求与使用边界。全新的旅行规划技能提供了完整的旅行工作流，包括 Xiaohongshu 集成和改进的路由选择逻辑，为用户提供从规划到执行的全流程支持。从 Python 到 JavaScript 的迁移使得旅行规划技能具有更好的性能、类型安全和测试支持。遵循各技能的最佳实践与故障排除建议，可在保证安全性与稳定性的前提下高效完成自动化任务。

## 附录
- 快速参考
  - 安装与认证：按各技能的"安装指引"与"Setup/Workflow"执行
  - 参数收集：按技能"When to Use"收集必要字段
  - 输出格式：关注 JSON 输出与格式化选项（如 jq、format、j1、PNG）
  - 权限与门控：检查令牌/密钥与门控开关（如 Discord actions.*）
  - 旅行规划：使用 travel_db.mjs 管理偏好和行程数据
  - Xiaohongshu：支持多账号管理和远程 CDP 连接
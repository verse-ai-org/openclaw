...
- [旅行规划技能：SKILL.md](file://skills/travel-planner/SKILL.md)
- [旅行规划技能：数据库管理器 db.mjs](file://skills/travel-planner/scripts/db.mjs)
- [旅行规划技能：CLI 参数解析 cli_args.mjs](file://skills/travel-planner/scripts/cli_args.mjs)
- [旅行规划技能：路线规划 route-plan.mjs](file://skills/travel-planner/scripts/route-plan.mjs)
- [旅行规划技能：路线验证 route-validation.mjs](file://skills/travel-planner/scripts/route-validation.mjs)
- [旅行规划技能：计划生成 plan-generator.mjs](file://skills/travel-planner/scripts/plan-generator.mjs)
- [旅行规划技能：小红书证据构建 xhs-evidence-builder.mjs](file://skills/travel-planner/scripts/xhs-evidence-builder.mjs)
- [旅行规划技能：预订准备 booking-ready.mjs](file://skills/travel-planner/scripts/booking-ready.mjs)
- [旅行规划技能：简报 briefing.mjs](file://skills/travel-planner/scripts/briefing.mjs)
- [旅行规划技能：示例对话 example_dialogue.md](file://skills/travel-planner/references/example_dialogue.md)
- [旅行规划技能：测试文件 travel-planner-skill.test.ts](file://test/travel-planner-skill.test.ts)
...
**更新** 本指南现已更新以反映旅行规划技能从 Python 到 JavaScript 的迁移，新增 Node.js 开发模式和 CLI 开发指导，涵盖完整的 JavaScript 技术栈和模块化开发实践。
...
- 资源目录：
  - scripts/：可执行脚本（Node.js/JavaScript），适合重复性高、需要确定性的任务。
...
**更新** 旅行规划技能展示了完整的 Node.js 开发模式，包括模块化设计、CLI 参数解析、JSON 数据持久化等现代 JavaScript 实践。
...
**更新** 旅行规划技能展示了现代化的 JavaScript 目录结构，包含多个功能模块化的脚本文件，每个文件都有明确的职责分工。
...
- **更新** 旅行规划技能：展示完整的 JavaScript 开发生态，包括模块化脚本、CLI 参数处理、JSON 数据持久化、测试驱动开发等现代实践。
...
### 组件九：Node.js 开发模式与 CLI 指导（新增）
**更新** 旅行规划技能展示了完整的 Node.js 开发模式：

- **模块化架构**：每个功能职责分离到独立的 JavaScript 模块
- **CLI 参数解析**：统一的 `cli_args.mjs` 提供标准的命令行参数处理
- **JSON 数据持久化**：使用本地 JSON 文件存储偏好和行程数据
- **类型安全**：采用 ES6 模块语法和现代 JavaScript 特性
- **测试驱动**：完整的单元测试和集成测试覆盖

```mermaid
graph TB
subgraph "Node.js 开发模式"
A["cli_args.mjs<br/>统一参数解析"]
B["db.mjs<br/>数据持久化"]
C["route-plan.mjs<br/>路线规划"]
D["route-validation.mjs<br/>验证逻辑"]
E["plan-generator.mjs<br/>计划生成"]
F["xhs-evidence-builder.mjs<br/>证据构建"]
G["booking-ready.mjs<br/>预订准备"]
H["briefing.mjs<br/>简报生成"]
end
A --> B
A --> C
A --> D
...
图表来源
- [旅行规划技能：CLI 参数解析 cli_args.mjs:1-187](file://skills/travel-planner/scripts/cli_args.mjs#L1-L187)
- [旅行规划技能：数据库管理器 db.mjs:1-837](file://skills/travel-planner/scripts/db.mjs#L1-L837)

章节来源
- [旅行规划技能：SKILL.md:1-447](file://skills/travel-planner/SKILL.md#L1-L447)
- [旅行规划技能：CLI 参数解析 cli_args.mjs:1-187](file://skills/travel-planner/scripts/cli_args.mjs#L1-L187)
- [旅行规划技能：数据库管理器 db.mjs:1-837](file://skills/travel-planner/scripts/db.mjs#L1-L837)
...
- **更新** Node.js 开发故障排查：
  - 检查 Node.js 版本兼容性（旅行规划技能要求 Node.js 环境）
  - 验证模块导入路径正确性
  - 确认 JSON 文件权限和路径配置
  - 使用 `--help` 参数检查 CLI 命令格式
  - 检查 `TRAVEL_PLANNER_DB_DIR` 环境变量设置
...
## 结论
OpenClaw 的技能体系以 SKILL.md 为核心，结合资源目录与配置注入，实现"可发现、可控制、可扩展"的能力边界。通过标准化的初始化、校验、打包流程，可以高效地从零构建高质量技能，并在多平台、多会话环境中稳定运行。

**更新** 随着旅行规划技能从 Python 迁移到 JavaScript，OpenClaw 现已支持现代化的 Node.js 开发生态。新的开发模式提供了更好的模块化设计、统一的 CLI 参数处理、完善的测试框架和更高效的性能表现。建议在设计阶段即考虑上下文成本、安全边界与可维护性，以获得更好的用户体验与工程收益。
...
- **更新** Node.js 开发：采用模块化设计，使用 ES6 模块语法，实现 CLI 参数解析和 JSON 数据持久化。
...
**更新** Node.js 开发最佳实践：
- 使用 `cli_args.mjs` 统一处理命令行参数
- 采用模块化设计，每个功能职责单一
- 实现完整的错误处理和参数验证
- 编写全面的单元测试和集成测试
- 使用 TypeScript 类型定义提升代码质量
- 遵循 ES6+ 语法规范和模块化最佳实践
...
### Node.js 开发指南（新增）
**更新** 旅行规划技能展示了完整的 Node.js 开发流程：

#### 1. 项目初始化
```bash
# 创建技能目录
mkdir skills/travel-planner
cd skills/travel-planner
npm init -y
```

#### 2. CLI 参数处理
使用 `cli_args.mjs` 提供的标准参数解析功能：
- 支持 `--key=value` 格式的参数
- JSON 值可直接内联或通过 `@/path/to.json` 文件引用
- 自动参数验证和错误处理

#### 3. 数据持久化
使用 `db.mjs` 实现本地 JSON 文件存储：
- 用户偏好数据存储在 `~/.openclaw/agents/travel-planner/preferences.json`
- 行程数据存储在 `~/.openclaw/agents/travel-planner/trips.json`
- 支持完整的 CRUD 操作和数据验证

#### 4. 模块化架构
- `route-plan.mjs`：路线候选生成和选择
- `route-validation.mjs`：实时验证和风险评估
- `plan-generator.mjs`：计划骨架生成
- `booking-ready.mjs`：预订准备包生成
- `xhs-evidence-builder.mjs`：小红书证据规范化
- `briefing.mjs`：行前和每日简报
...
- [旅行规划技能：CLI 参数解析 cli_args.mjs:1-187](file://skills/travel-planner/scripts/cli_args.mjs#L1-L187)
- [旅行规划技能：数据库管理器 db.mjs:1-837](file://skills/travel-planner/scripts/db.mjs#L1-L837)
- [旅行规划技能：路线规划 route-plan.mjs:1-285](file://skills/travel-planner/scripts/route-plan.mjs#L1-L285)
- [旅行规划技能：路线验证 route-validation.mjs:1-382](file://skills/travel-planner/scripts/route-validation.mjs#L1-L382)
- [旅行规划技能：计划生成 plan-generator.mjs:1-703](file://skills/travel-planner/scripts/plan-generator.mjs#L1-L703)
- [旅行规划技能：小红书证据构建 xhs-evidence-builder.mjs:1-183](file://skills/travel-planner/scripts/xhs-evidence-builder.mjs#L1-L183)
- [旅行规划技能：预订准备 booking-ready.mjs:1-260](file://skills/travel-planner/scripts/booking-ready.mjs#L1-L260)
- [旅行规划技能：简报 briefing.mjs:1-149](file://skills/travel-planner/scripts/briefing.mjs#L1-L149)
- [旅行规划技能：示例对话 example_dialogue.md:1-76](file://skills/travel-planner/references/example_dialogue.md#L1-L76)
- [旅行规划技能：测试文件 travel-planner-skill.test.ts:1-429](file://test/travel-planner-skill.test.ts#L1-L429)
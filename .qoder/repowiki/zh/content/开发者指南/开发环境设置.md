...
## 更新摘要
**变更内容**
- 新增 VITE_GATEWAY_TOKEN 环境变量支持，简化本地开发环境配置
- 改进开发令牌解析机制，支持多层级令牌优先级
- 优化前端开发体验，减少手动配置步骤
- 更新环境变量加载顺序和优先级说明
...
**更新** 新增 VITE_GATEWAY_TOKEN 环境变量支持，显著简化了本地开发环境配置流程。
...
### 步骤 3: 环境变量配置
**新增** VITE_GATEWAY_TOKEN 环境变量配置：

```bash
# 创建本地开发配置文件
mkdir -p ui-react/.env.local

# 在 ui-react/.env.local 中添加开发令牌
echo 'VITE_GATEWAY_TOKEN=your_gateway_token_here' >> ui-react/.env.local

# 或者设置环境变量
export VITE_GATEWAY_TOKEN=your_gateway_token_here
```
...
# 验证 VITE_GATEWAY_TOKEN 配置
echo $VITE_GATEWAY_TOKEN
...
### VITE_GATEWAY_TOKEN 配置问题
**问题**: VITE_GATEWAY_TOKEN 未生效或配置错误
**解决方案**:
```bash
# 检查 .env.local 文件是否存在
ls -la ui-react/.env.local

# 验证 .env.local 格式
cat ui-react/.env.local
...
### 环境变量加载顺序
**更新** 新增 VITE_GATEWAY_TOKEN 环境变量支持，改进了令牌解析机制：
...
Process[进程环境变量]
DotEnv[CWD .env 文件]
GlobalEnv[全局 ~/.openclaw/.env]
ConfigEnv[配置文件 env 块]
ShellImport[登录壳导入]
end
Process --> DotEnv --> GlobalEnv --> ConfigEnv --> ShellImport
```

### VITE_GATEWAY_TOKEN 令牌优先级
**新增** VITE_GATEWAY_TOKEN 环境变量为开发模式提供便捷的令牌配置：

```mermaid
flowchart TD
subgraph "令牌解析优先级"
URLParam[URL 参数 token]
SessionStorage[会话存储 token]
VITE_GATEWAY_TOKEN[开发令牌 VITE_GATEWAY_TOKEN]
ConfigToken[配置文件 token]
end
URLParam --> SessionStorage --> VITE_GATEWAY_TOKEN --> ConfigToken
```

### 开发环境配置
**新增** VITE_GATEWAY_TOKEN 环境变量配置方法：
...
# 方法3: 在 .env 文件中设置
echo 'VITE_GATEWAY_TOKEN=your_gateway_token_here' > ui-react/.env
```

### 环境变量安全
**更新** 环境变量加载安全性增强：

- **非覆盖原则**: 新环境变量不会覆盖已存在的变量
- **安全加载**: `.env` 文件位于当前工作目录，不会覆盖全局设置
- **配置块保护**: 配置文件中的 `env` 块只在缺失时应用

**章节来源**
- [docs/help/environment.md:14-22](file://docs/help/environment.md#L14-L22)
- [ui-react/vite.config.ts:21-26](file://ui-react/vite.config.ts#L21-L26)
- [ui-react/src/store/settings.store.ts:114-124](file://ui-react/src/store/settings.store.ts#L114-L124)

## 结论

OpenClaw 项目提供了完整的开发环境设置方案，涵盖了从基础环境配置到高级调试技巧的各个方面。通过遵循本文档的指导，开发者可以快速搭建起稳定高效的开发环境，并充分利用项目提供的现代化工具链和架构设计。

**更新** 新增的 VITE_GATEWAY_TOKEN 环境变量支持显著简化了本地开发配置流程，通过多层级令牌优先级机制，为开发者提供了更加灵活和便捷的开发体验。
...
- **VITE_GATEWAY_TOKEN 环境变量**简化了本地开发配置
- **改进的令牌解析机制**提升了开发体验和安全性

建议开发者在开始项目开发前，仔细阅读并实践本文档中的所有步骤，特别注意 VITE_GATEWAY_TOKEN 环境变量的配置，以确保获得最佳的开发体验。
...
## 更新摘要
**所做更改**
- 新增MiniMax M2.5变体模型支持章节，包括三个变体：标准版、高速版和Lightning版
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [MiniMax M2.5变体模型支持](#minimax-m25变体模型支持)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
...
- **新增：MiniMax变体模型配置**：支持MiniMax M2.5标准版、高速版和Lightning三个变体的配置与管理。
...
### 变体概览
OpenClaw现已全面支持MiniMax M2.5系列的三个变体模型：
...
- **MiniMax M2.5 Lightning**（Lightning版）
  - 最高性能版本
  - 默认reasoning设置：true
  - 成本：0.3/1.2（输入/输出，单位：美元/百万tokens）
  - 上下文窗口：200,000
...
- 运行时快照与覆盖注入保障配置的即时生效
- 跨平台与跨运行时的一致性与安全性
- **新增：MiniMax M2.5变体模型的全面支持，包括三个性能等级的模型配置与OAuth认证流程**

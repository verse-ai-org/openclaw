...
- 新增了从 SessionSelector 组件到 useSessionManager 钩子的架构变更说明
- 添加了 ChatSidebar 中 useSessionManager 集成的详细分析
- 更新了 UI 层会话管理的实现细节和组件交互
- 增强了会话管理在前端 React 应用中的集成文档
...
**重要变更**：会话管理架构已从传统的 SessionSelector 组件重构为基于 React Hooks 的 useSessionManager 钩子模式，ChatSidebar 作为主要的 UI 组件直接集成了该钩子。
...
会话管理围绕"会话存储文件 + 会话条目 + 维护策略 + 缓存/锁队列 + 元数据派生 + 网关/工具集成 + UI 集成"的闭环展开。写入路径在保存前执行维护，读取路径支持缓存；维护策略可警告或强制执行；磁盘预算在强制模式下按旧文件与旧会话条目清理。

**重要架构变更**：UI 层采用 React Hooks 模式，useSessionManager 钩子集中管理会话状态，ChatSidebar 直接消费该钩子的状态和方法。
...
OpenClaw 会话管理通过严谨的数据模型、完善的维护策略与缓存/锁机制，在保证一致性的同时兼顾性能与可运维性。**最新的架构变更**采用 React Hooks 模式，useSessionManager 钩子提供了更加模块化和可测试的会话管理方案，ChatSidebar 直接集成了该钩子，实现了更好的用户体验和开发效率。结合 CLI 与网关查询能力，用户可以高效地观察、清理与优化会话状态。

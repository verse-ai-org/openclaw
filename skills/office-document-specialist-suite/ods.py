...
def cmd_template_report(output: Path, title: str, author: str):
    ...
    p2 = doc.add_paragraph(f"Author: {author}\nDate: {date.today().isoformat()}")
    p2.alignment = WD_ALIGN_PARAGRAPH.CENTER

    doc.add_page_break()
    doc.add_paragraph("Table of Contents", style="Heading 1")
    doc.add_paragraph("(Update the table of contents in Word/LibreOffice: right-click > Update Field)")

    doc.add_page_break()
    doc.add_paragraph("1. Executive Summary", style="Heading 1")
    doc.add_paragraph("Insert management summary here.")

    doc.add_paragraph("2. Analysis", style="Heading 1")
    doc.add_paragraph("Use level 2 and 3 headings for structure.")

    quote = doc.add_paragraph("Key observation or quote.", style="Quote Accent")
    quote.paragraph_format.left_indent = Cm(1)

    doc.add_paragraph("2.1 Detail Section", style="Heading 2")
    doc.add_paragraph("Place tables, figures, and technical details here.")
...
def cmd_style_doc(input_file: Path, output_file: Path, landscape: bool):
    ...
    print(f"Styles applied: {output_file}")
...
def build_parser():
    ...
    rep = sub.add_parser("template-report", help="Create advanced report template (.docx)")
    rep.add_argument("--output", default="report-template.docx", type=Path)
    rep.add_argument("--title", default="Report Title")
    rep.add_argument("--author", default="Unknown")

    sty = sub.add_parser("style-doc", help="Apply advanced styling to existing .docx")

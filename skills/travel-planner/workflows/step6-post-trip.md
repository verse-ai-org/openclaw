# Step 6 - Post-trip（行后沉淀：归档与偏好回写）

本文件覆盖主流程 Step 6（F-1 ~ F-3）。
...
```bash
node {baseDir}/scripts/preferences.mjs --cmd=save \
  --payload='{"previous_destinations": ["<城市, 国家>"]}'
```

## F-3｜更新复用偏好

根据本次行程真实反馈，更新后调用 `save`：

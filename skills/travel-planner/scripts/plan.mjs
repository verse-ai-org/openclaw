...
import { requirePlanDepthChoice, requireStageAtLeast } from "./lib/guards.mjs";
...
function main() {
  ...
  try {
    if (!cmd || cmd === "help") {
      okJson({ commands: ["save_overview", "save_details", "get_inputs"] });
      return;
    }
    ...
    if (cmd === "save_details") {
      ...
      const gate = requireStageAtLeast(trip, STAGES.VALIDATED);
      if (!gate.ok) return failJson("guard failed", { reasons: gate.reasons });
      const depthGate = requirePlanDepthChoice(trip, "full_plan");
      if (!depthGate.ok) return failJson("guard failed", { reasons: depthGate.reasons });

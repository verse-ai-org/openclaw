...
**Tell each subagent:**
1. File naming: `slides/slide-01.js`, `slides/slide-02.js`, etc. (or `.cjs`)
...
Create `slides/compile.cjs` to combine all slide modules:
...
for (let i = 1; i <= 12; i++) {  // adjust count as needed
  ...
  const slideModule = require(`./slide-${num}.js`); // or .cjs
  slideModule.createSlide(pres, theme);
}

// Runtime will write to absolute path and print file info
pres.writeFile({ fileName: process.env.BOSSIM_PPTX_OUTPUT_PATH || "./output/presentation.pptx" });
```
...
### Step 8: Completion Gate (Mandatory)

Do not end the task after writing slide source files. A PPTX task is complete only after final compilation succeeds and output is confirmed on disk.
...
If compile fails, do not claim completion. Return:
- concise error message,
- error log path (if available),
- next action you will take to recover.
...
└── output/              # Local test output only (if manually overriding path)
    └── presentation.pptx
```

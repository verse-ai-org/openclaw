...
default_pdf_dir() {
  ...
  # - macOS/Linux: $HOME/Documents/Bossim/Pdf
  # - Windows (Git Bash/MSYS/Cygwin): prefer $HOME, fallback USERPROFILE
  local home_dir="${HOME:-}"
...
ensure_default_out_path() {
  ...
  local out_dir
  out_dir="$(default_pdf_dir)"
  mkdir -p "$out_dir"
  printf "%s/%s" "$out_dir" "$default_name"
}
...
cmd_run() {
  ...
  out="$(ensure_default_out_path "$out" "output.pdf")"
...
cmd_fill() {
  ...
  out="$(ensure_default_out_path "$out" "filled.pdf")"

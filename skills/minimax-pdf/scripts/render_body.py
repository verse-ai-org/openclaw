...
def _walk_has_cjk(value) -> bool:
    ...
    if isinstance(value, dict):
        return any(_walk_has_cjk(v) for v in value.values())
    if isinstance(value, list):
        return any(_walk_has_cjk(v) for v in value)
    return False


def _register_cjk_fallback_font() -> str | None:
    """
    Register a CJK-capable font and return the ReportLab font name.
    Priority:
    1) common system fonts (macOS/Linux)
...
def make_styles(t: dict) -> dict:
    ...
    return {
        ...
        "code": ParagraphStyle("Code",
            fontName=code_font, fontSize=8.5, leading=12.5,
            textColor=HexColor(dk),
        ),
        "code_lang": ParagraphStyle("CodeLang",
            fontName=code_font, fontSize=7, leading=10,
...
def build(tokens: dict, content: list, out_path: str) -> dict:
    ...
    if _walk_has_cjk(content) or _walk_has_cjk({
        ...
        cjk_font = _register_cjk_fallback_font()
        if cjk_font:
            # Keep a consistent CJK-capable font for all text styles to avoid glyph loss.

...
## Output Path Default Rule

If the user does not explicitly provide an output file path, save outputs under:
- macOS/Linux: `/Documents/Bossim/Excel/`
- Windows: `C:\Users\<username>\Documents\Bossim\Excel\`

When this default is used, create the directory if it does not exist and choose a clear filename based on task type (for example: `analysis-report.xlsx`, `edited-output.xlsx`, `formula-check-report.json`).

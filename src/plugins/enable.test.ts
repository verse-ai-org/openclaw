...
import { enableChannelInConfig, enablePluginInConfig } from "./enable.js";
...
describe("enableChannelInConfig", () => {
  it("adds a meaningful channels section for external channel plugins", () => {
    const result = enableChannelInConfig({} as OpenClawConfig, "openclaw-weixin");

    expect(result.enabled).toBe(true);
    expect(result.config.plugins?.entries?.["openclaw-weixin"]?.enabled).toBe(true);
    expect(result.config.channels?.["openclaw-weixin"]).toEqual({
      enabled: true,
      accounts: { default: {} },
    });
  });

  it("preserves existing external channel config keys", () => {
    const result = enableChannelInConfig(
      {
        channels: {
          "openclaw-weixin": {
            accounts: { work: { token: "x" } },
          },
        },
      } as OpenClawConfig,
      "openclaw-weixin",

...
/** UI-facing plugin row: core PluginRecord plus gateway RPC methods derived from the registry. */
export type PluginStatusApiRecord = PluginRecord & {
  gatewayMethods: string[];
};

export function enrichPluginsForStatusApi(report: PluginStatusReport): PluginStatusApiRecord[] {
  const descriptors = report.gatewayMethodDescriptors ?? [];
  return report.plugins.map((plugin) => ({
    ...plugin,
    gatewayMethods: descriptors
      .filter(
        (descriptor) =>
          descriptor.owner.kind === "plugin" && descriptor.owner.pluginId === plugin.id,
      )

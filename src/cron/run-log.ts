...
function normalizeSinceMs(sinceMs?: number): number | undefined {
  return typeof sinceMs === "number" && Number.isFinite(sinceMs)
...
export async function readCronRunLogEntriesPage(
  ...
  const sinceMs = normalizeSinceMs(opts.sinceMs);

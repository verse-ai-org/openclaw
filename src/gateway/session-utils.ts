  ...
  for (const entry of cfg.agents?.list ?? []) {
    ...
    let identity = entry.identity
      ? {
          ...
          video: normalizeOptionalString(entry.identity.video),
          bio: normalizeOptionalString(entry.identity.bio),
        }
      : undefined;
      ...
      try {
        ...
        if (resolved) {
          identity = {
            ...
            emoji: identity?.emoji ?? resolved.emoji,
            avatar: identity?.avatar ?? resolved.avatar,
            avatarUrl:
              identity?.avatarUrl ??
        ...
        // Keep partial identity from config when IDENTITY.md is unavailable.
      }
    }

...
import { FsSafeError, readFileWithinRoot, writeFileWithinRoot } from "../../infra/fs-safe.js";
...
function resolveSkillStatusEntry(
  baseDir: string,
  source: string,
):
  | {
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.file.get": async ({ params, respond }) => {
    if (!assertValidParams(params, validateSkillsFileGetParams, "skills.file.get", respond)) {
      return;
    }
    const entry = resolveSkillStatusEntry(params.baseDir, params.source);
    if (!entry) {
      respond(
        false,
        undefined,
    ...
    const relativePath = path.relative(entry.baseDir, path.join(entry.baseDir, "SKILL.md"));
    try {
      ...
      if (err instanceof FsSafeError && err.code === "not-found") {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "SKILL.md not found"));
        return;
      }
      respond(
        false,
  ...
  "skills.file.set": async ({ params, respond }) => {
    if (!assertValidParams(params, validateSkillsFileSetParams, "skills.file.set", respond)) {
      return;
    }
    const entry = resolveSkillStatusEntry(params.baseDir, params.source);
    if (!entry) {
  ...
  "skills.import": async ({ params, respond }) => {
    if (!assertValidParams(params, validateSkillsImportParams, "skills.import", respond)) {
      return;
    }
    const cfg = loadConfig();
    const target = params.target ?? "managed";
  ...
  "skills.remove": async ({ params, respond }) => {
    if (!assertValidParams(params, validateSkillsRemoveParams, "skills.remove", respond)) {
      return;
    }
    const result = await removeSkill({ baseDir: params.baseDir, source: params.source });
    if (result.ok) {
      bumpSkillsSnapshotVersion({ reason: "manual" });
    }
    respond(

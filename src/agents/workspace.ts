...
/** @deprecated Use isWorkspaceSetupCompleted */
export const isWorkspaceOnboardingCompleted = isWorkspaceSetupCompleted;
...
export async function ensureAgentWorkspace(params?: {
  ...
  const soulTemplate = await loadTemplateWithFallback(DEFAULT_SOUL_FILENAME, templateSubdir);
  const toolsTemplate = await loadTemplateWithFallback(DEFAULT_TOOLS_FILENAME, templateSubdir);
  const identityTemplate = await loadTemplateWithFallback(
    DEFAULT_IDENTITY_FILENAME,
    templateSubdir,
  );
  const userTemplate = await loadTemplateWithFallback(DEFAULT_USER_FILENAME, templateSubdir);

...
export const weixinPlugin: ChannelPlugin<ResolvedWeixinAccount> = {
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId }) => {
      ...
      if (rawId) {
        clearContextTokensForAccount(rawId);
      }
      clearWeixinQrLoginSessions(normalizedId);
      if (rawId) {
        clearWeixinQrLoginSessions(rawId);
      }
      const { cleared } = logoutWeixinStoredAccount(normalizedId);
      if (cleared) {
        await triggerWeixinChannelReload();
      }
      logger.info(`logoutAccount: accountId=${normalizedId} cleared=${cleared}`);
    ...
    loginWithQrWait: async (params) => {
      ...
      const result: WeixinQrWaitResult = await waitForWeixinLogin({
        ...
        verifyCode: (params as { verifyCode?: string }).verifyCode,
      });
      ...
      return {
        ...
        needsVerifyCode: result.needsVerifyCode,

...
describe("logoutWeixinStoredAccount", () => {
  it("removes credentials, index entry, and legacy token file", async () => {
    const {
      saveWeixinAccount,
      registerWeixinAccountId,
      logoutWeixinStoredAccount,
      loadWeixinAccount,
      listIndexedWeixinAccountIds,
    } = await loadModule();
    saveWeixinAccount("acc-logout", { token: "tok" });
    registerWeixinAccountId("acc-logout");
    const legacyDir = path.join(tmpDir, "credentials", "openclaw-weixin");
    fs.mkdirSync(legacyDir, { recursive: true });
    fs.writeFileSync(path.join(legacyDir, "credentials.json"), JSON.stringify({ token: "legacy" }));

    const result = logoutWeixinStoredAccount("acc-logout");

    expect(result.cleared).toBe(true);
    expect(loadWeixinAccount("acc-logout")).toBeNull();
    expect(listIndexedWeixinAccountIds()).not.toContain("acc-logout");

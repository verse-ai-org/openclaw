...
/** Drop in-memory QR login state (logout or account switch). */
export function clearWeixinQrLoginSessions(sessionKey?: string): void {
  if (sessionKey?.trim()) {
    activeLogins.delete(sessionKey.trim());
    return;
  }
  activeLogins.clear();
}
...
export type WeixinQrWaitResult = {
  ...
  /** Updated QR link after server-side expiry refresh; UI should redraw and call wait again. */
  qrDataUrl?: string;
  /** Pair-code challenge: gateway/UI must collect digits and pass `verifyCode` on the next wait. */
  needsVerifyCode?: boolean;
  /** Slice wait timed out but QR session is still active; UI should call wait again. */
  pending?: boolean;
  botToken?: string;
  accountId?: string;
  baseUrl?: string;
...
function returnRefreshedQr(activeLogin: ActiveLogin): WeixinQrWaitResult {
  return {
    connected: false,
    pending: true,
    qrDataUrl: activeLogin.qrcodeUrl,
    message: "二维码已更新，请用微信重新扫描。",
  };
}

function isSliceWaitTimeout(timeoutMs: number): boolean {
  return timeoutMs <= 90_000;
}

function returnSlicePending(activeLogin: ActiveLogin): WeixinQrWaitResult {
  const status = activeLogin.status;
  const message =
    status === "scaned"
      ? "已扫码，正在确认…"
...
export async function waitForWeixinLogin(opts: {
  ...
  if (opts.verifyCode?.trim()) {
    activeLogin.pendingVerifyCode = opts.verifyCode.trim();
  }
    ...
    try {
      ...
      switch (statusResponse.status) {
        ...
        case "scaned": {
          ...
          // Return quickly so Control UI can show "已扫码" instead of waiting for slice timeout.
          if (!process.stdin.isTTY) {
            return returnSlicePending(activeLogin);
          }
          break;
        }
        case "need_verifycode": {
          if (!activeLogin.pendingVerifyCode) {
            if (!process.stdin.isTTY) {
              logger.info("waitForWeixinLogin: need_verifycode in non-interactive mode");
              return {
                connected: false,
                needsVerifyCode: true,
                message: "请输入手机微信显示的配对数字，然后在界面提交后继续。",
              };
            }
            const verifyPrompt = "输入手机微信显示的数字，以继续连接：";
          ...
          if (!process.stdin.isTTY) {
            return {
              connected: false,
              needsVerifyCode: true,
              message: "配对数字不正确，请重新输入手机微信显示的数字。",
            };
          }
          const verifyPrompt = "❌ 你输入的数字不匹配，请重新输入：";
          const code = await readVerifyCodeFromStdin(verifyPrompt);
          activeLogin.pendingVerifyCode = code;
          continue;
        }
        case "expired": {
          ...
          if (!process.stdin.isTTY) {
            return returnRefreshedQr(activeLogin);
          }
          break;
        }
  ...
  if (isSliceWaitTimeout(timeoutMs)) {
    logger.info(
      `waitForWeixinLogin: slice wait elapsed, keeping session sessionKey=${opts.sessionKey} status=${activeLogin.status ?? "unknown"}`,
    );
    return returnSlicePending(activeLogin);
  }
